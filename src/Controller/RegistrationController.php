...
class RegistrationController extends AbstractController
{
    ...
    public function register(Request $request, UserPasswordHasherInterface $userPasswordHasher, UserAuthenticatorInterface $userAuthenticator, UserAuthenticator $authenticator, EntityManagerInterface $entityManager, MailerInterface $mailer): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $email = (new Email())
                ->from(new Address('database85170@gmail.com', 'Nicolas Cournau'))
                ->to($user->getEmail())
                //->from(new Address('database85170@gmail.com', 'Nicolas Cournau'))
                //->to('userrandom5@gmail.com')
                //->cc('cc@example.com')
                //->bcc('bcc@example.com')
                //->replyTo('fabien@example.com')
                //->priority(Email::PRIORITY_HIGH)
                ->subject('Inscription')
                ->text('Voici le mail de confirmation de création de compte')
                ->html('<p>Voici le mail de confirmation de création de compte</p>');
